...
REGIONS = [
    ...
    ("Taiwan", [116, 126, 19, 24])
]

# Logo 路徑
LOGO_PATH = "./logo.png"  # 請確保此檔案存在於根目錄
...
def add_title_to_map(ax, title_text, dt_utc):
    """在地圖上加入標題（轉為 UTC+8）"""
    # 時間轉換 UTC -> UTC+8
    dt_local = dt_utc + timedelta(hours=8)
    time_str = f"{dt_local.strftime('%Y-%m-%d %H:%M')} (UTC+8)"

    # 主標題
    ax.text(0.02, 1.02, title_text, 
    ...
    # 副標題 (時間)
    ax.text(0.98, 1.02, time_str,
            transform=ax.transAxes,
            fontsize=12, family='serif',
            ha='right', va='bottom',
            color='yellow', # 時間用黃色凸顯
...
def add_logo_to_map(ax):
    ...
    if os.path.exists(LOGO_PATH):
        try:
            ...
            # zoom 控制 Logo 大小，您可能需要根據 Logo 解析度調整這個數值 (例如 0.1 或 0.5)
            imagebox = OffsetImage(logo_img, zoom=0.15, alpha=0.8) 
            
            # xy=(1, 0) 代表 Axes 的右下角, xybox 用於微調位置
            ab = AnnotationBbox(imagebox, (1, 0), xycoords='axes fraction',
                                box_alignment=(1.0, 0.0), # 對齊右下
                                frameon=False)
            ax.add_artist(ab)
        except Exception as e:
            print(f"無法載入 Logo: {e}")
...
def process_single_band(ds, band_id, band_info, lon, lat, dt_utc, output_folder):
    ...
    # 讀取資料
    arr = ds[vname].values
    if arr.ndim == 3: arr2d = arr[0, ...]
    else: arr2d = arr
    
    # 準備繪圖資料 (灰階用正規化 uint8，紅外線彩色用原始資料)
    img_uint8 = normalize_uint8(arr2d, invert=invert)
    ...
    # 針對每個設定的區域 (EastAsia, Taiwan) 進行迴圈
    for region_name, extent in REGIONS:
        
        for cmap_type in tasks:
            # 初始化參數
            plot_data = img_uint8
            ...
            if cmap_type == "color":
                ...
                if not settings: continue # 如果該波段沒有定義彩色設定則跳過
                ...
                if settings.get("use_raw_data", False):
                    plot_data = arr2d
                    plot_vmin = settings.get("vmin")
                    plot_vmax = settings.get("vmax")

            # 建立地圖
            fig = plt.figure(figsize=(10, 10)) # 根據需要調整大小
            ax = plt.axes(projection=ccrs.PlateCarree())
            ax.set_extent(extent, ccrs.PlateCarree())
            
            # 地圖特徵
            ax.coastlines(resolution='10m', linewidth=1.0, color=coastline_color) # 台灣範圍建議用 10m
            ax.add_feature(cfeature.BORDERS.with_scale('10m'), linewidth=0.8, edgecolor=coastline_color)
            ...
            if cmap_type == "color":
                ...
                if band_type == "infrared":
                    ...
                    # Kelvin 轉 Celsius
                    cbar.ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, pos: f"{x - 273.15:.0f}"))
            
            # 加入標題 & Logo
            title_txt = f"{band_name}"
            add_title_to_map(ax, title_txt, dt_utc)
            add_logo_to_map(ax)
            
            # 檔名處理 (加入區域後綴)
            # 如果是 EastAsia (原始設定)，不加後綴以符合您的檔名要求，或者您可以統一加
            region_suffix = "" if region_name == "EastAsia" else f"_{region_name}"
            
            # 依據您的需求命名: B03_original.png, B08_original_color.png
            filename = f"{band_id}_original{suffix}{region_suffix}.png"
            final_path = os.path.join(output_folder, filename)
            
            # 儲存
            bg = 'black' if cmap_type == "gray" else '#1a1a1a'
            plt.savefig(final_path, bbox_inches='tight', dpi=200, facecolor=bg)
            plt.close()
